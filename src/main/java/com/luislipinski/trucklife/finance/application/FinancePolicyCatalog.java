...
final class FinancePolicyCatalog {
    ...
    private static final Map<String, BigDecimal> ATS_STATE_FACTORS = Map.ofEntries(
            Map.entry("AZ",bd("0.78")),Map.entry("AR",bd("0.58")),Map.entry("CA",bd("1")),Map.entry("CO",bd("0.88")),
            Map.entry("ID",bd("0.72")),Map.entry("IL",bd("0.75")),Map.entry("IA",bd("0.62")),Map.entry("KS",bd("0.62")),
            Map.entry("LA",bd("0.65")),Map.entry("MO",bd("0.65")),Map.entry("MT",bd("0.72")),Map.entry("NE",bd("0.65")),
            Map.entry("NV",bd("0.82")),Map.entry("NM",bd("0.65")),Map.entry("OK",bd("0.60")),Map.entry("OR",bd("0.86")),
            Map.entry("TX",bd("0.70")),Map.entry("UT",bd("0.78")),Map.entry("WA",bd("0.95")),Map.entry("WY",bd("0.67"))
    );
    ...
    private static final Map<String, CountryProfile> ETS2 = Map.ofEntries(
            ...
            Map.entry("XK",effective("EUR","1","0.48")),Map.entry("MK",effective("MKD","61.5","0.44")),Map.entry("AL",effective("ALL","92.60","0.46")),
    ...
    static Map<MonthlyExpenseCategory, BigDecimal> defaults(CareerEntity career) {
        BigDecimal cityFactor = positiveOrOne(career.getCityCostFactor());
        BigDecimal displayExchange = positiveOrOne(career.getExchangeRate());
        ...
        if (career.getGame() == CareerGame.ATS) {
            ...
            for (MonthlyExpenseCategory category : MonthlyExpenseCategory.values()) {
                BigDecimal amount = ATS_BASE.get(category).multiply(stateFactor);
                if (category.citySensitive()) amount = amount.multiply(cityFactor);
                result.put(category, money(amount.multiply(displayExchange)));
            }
            return result;
        }
        ...
        for (MonthlyExpenseCategory category : MonthlyExpenseCategory.values()) {
            BigDecimal amount = country.amount(category);
            if (category.citySensitive()) amount = amount.multiply(cityFactor);
            result.put(category, money(amount.multiply(displayExchange)));
        }
        return result;
    ...
    private static CountryProfile direct(String currency, Map<MonthlyExpenseCategory, BigDecimal> amounts) {
        return new CountryProfile(currency, Map.copyOf(amounts));
    }
    private static CountryProfile effective(String currency, String perEuro, String costFactor) {
        Map<MonthlyExpenseCategory, BigDecimal> values = new EnumMap<>(MonthlyExpenseCategory.class);
        BigDecimal exchange=bd(perEuro), factor=bd(costFactor);
        EUR_BASE.forEach((category, amount) -> values.put(category, money(amount.multiply(factor).multiply(exchange))));
        return direct(currency, values);
    }
    private static Map<MonthlyExpenseCategory, BigDecimal> expenses(String rent,String electricity,String water,String internet,String phone,String groceries,String eatingOut,String health,String publicTransport,String household,String leisure) {
        Map<MonthlyExpenseCategory, BigDecimal> map = new EnumMap<>(MonthlyExpenseCategory.class);
        map.put(MonthlyExpenseCategory.RENT,bd(rent)); map.put(MonthlyExpenseCategory.ELECTRICITY,bd(electricity)); map.put(MonthlyExpenseCategory.WATER,bd(water));
        map.put(MonthlyExpenseCategory.INTERNET,bd(internet)); map.put(MonthlyExpenseCategory.PHONE,bd(phone)); map.put(MonthlyExpenseCategory.GROCERIES,bd(groceries));
        map.put(MonthlyExpenseCategory.EATING_OUT,bd(eatingOut)); map.put(MonthlyExpenseCategory.HEALTH,bd(health)); map.put(MonthlyExpenseCategory.PUBLIC_TRANSPORT,bd(publicTransport));
