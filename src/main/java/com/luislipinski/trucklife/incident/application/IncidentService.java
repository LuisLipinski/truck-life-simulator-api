...
public class IncidentService implements IncidentOperations {
    ...
    private final IncidentPayslipDeductionRepository deductionRepository;
    ...
    public IncidentService(
            ...
            IncidentPayslipDeductionRepository deductionRepository,
            Clock clock
    ) {
    ...
    public Result create(
        ...
        if (expectedOperationalWeek != career.getCurrentOperationalWeek()) {
            throw conflict(
                    "INCIDENT_WEEK_CONFLICT",
                    "Operational week changed",
                    "The requested week is no longer the career current operational week"
            );
        }

        TripEntity trip = relatedTripId == null
                ? null
                : tripRepository.findByIdAndCareerId(relatedTripId, career.getId())
                        .orElseThrow(() -> new ResourceNotFoundException(
                                "TRIP_NOT_FOUND",
                                "The related trip does not exist for this career"
                        ));

        int incidentWeek = trip == null
                ? career.getCurrentOperationalWeek()
                : trip.getOperationalWeek();
        String routeLabel = trip == null
                ? routeLabel(route)
                : trip.getOriginCity() + " → " + trip.getDestinationCity();
        BigDecimal normalizedAmount = amount.setScale(2, RoundingMode.UNNECESSARY);
        Instant now = clock.instant();
        IncidentStatus status = chargeMethod == IncidentChargeMethod.BALANCE
                ? IncidentStatus.PAID_BALANCE
                : IncidentStatus.PENDING_PAYSLIP;
        BigDecimal remaining = chargeMethod == IncidentChargeMethod.BALANCE
        ...
        if (chargeMethod == IncidentChargeMethod.BALANCE) {
            career.debitBalance(normalizedAmount, now);
        }
    ...
    public List<Result> list(UUID userId, CareerGame game, UUID careerId) {
        ...
        return incidentRepository.findAllByCareerIdOrderByRecordedAtDescIdDesc(career.getId())
                .stream()
                .map(this::result)
                .toList();
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Result get(UUID userId, CareerGame game, UUID careerId, UUID incidentId) {
        CareerEntity career = ownedCareer(userId, game, careerId);
        IncidentEntity incident = incidentRepository.findByIdAndCareerId(incidentId, career.getId())
                .orElseThrow(() -> new ResourceNotFoundException(
                        "INCIDENT_NOT_FOUND",
    ...
    @Override
    @Transactional
    public void cancel(UUID userId, CareerGame game, UUID careerId, UUID incidentId) {
        CareerEntity career = lockedOwnedCareer(userId, game, careerId);
        IncidentEntity incident = incidentRepository.findByIdAndCareerId(incidentId, career.getId())
                .orElseThrow(() -> new ResourceNotFoundException(
                        "INCIDENT_NOT_FOUND",
        ...
        if (!incident.canCancel()) {
            throw conflict(
                    "INCIDENT_NOT_CANCELLABLE",
                    "Incident cannot be cancelled",
                    "Only an untouched incident pending for a future payslip can be cancelled"
            );
        }
        incident.cancel(clock.instant());
        incidentRepository.flush();
    }

    private Result result(IncidentEntity incident) {
        return new Result(
                incident,
                deductionRepository.findAllByIncidentIdOrderByRecordedAtAscIdAsc(incident.getId())
        );
    }

    private String routeLabel(String route) {
        return route == null || route.isBlank() ? "Manual route" : route.strip();
    }
